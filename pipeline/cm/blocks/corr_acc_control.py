...
class CorrAcc(Corr):
    def get_next_allowed_start(self, delay_s):
        ...
        self.log.debug("Detected sync time %s" % time.ctime(sync_time))
        spectra_rate = status['bw_hz'] / status['nchan']
        self.log.debug("Computed spectra rate is %d spectra / second" % spectra_rate)
        last_count = status["stats"]["curr_sample"]
        rough_now = sync_time + (last_count / spectra_rate)
        diff = rough_now - time.time()
        if abs(diff) > 10:
            self.log.warning("It looks like the pipeline is %d seconds ahead of the expected spectra number" % diff)
        spectra_delay = (time.time() + delay_s - sync_time) * spectra_rate
        self.log.debug("Spectra delay is %f" % spectra_delay)
        ...
        self.log.debug("Spectra delay after rounding to %d is %d" % (upstream_acc_len, rounded_spectra_delay))
        rounded_spectra_delay += offset
        self.log.debug("Spectra delay after applying offset of %d is %d" % (offset, rounded_spectra_delay))
