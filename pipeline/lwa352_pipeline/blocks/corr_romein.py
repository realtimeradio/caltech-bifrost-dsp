...
class Corr_Romein(Block):
    ...
    **Runtime Control and Monitoring**
    ...
    def __init__(self, log, iring, oring, conv, grid, ntime_gulp=2500,
        ...
        super(Corr_Romein, self).__init__(log, iring, oring, guarantee, core, etcd_client=etcd_client)
        ...
        self.grid_out = self.grid_size**2 * 2 * 8
    ...
    def main(self):
        ...
        self.log.info("Attempting to allocate %d bytes of GPU memory for gridder output" % self.grid_out)
            ...
            for iseq in self.iring.read(guarantee=self.guarantee):
                ...
                for ispan in iseq.read(self.igulp_size):
                    ...
                    if this_gulp_time == first:
                        ...
                        # try by channel [32] x baselines x npol [4]
                        #self.log.info("Attempting to allocate %d bytes of GPU memory for correlator" % (self.nchan*2099200*4*8))
                        #correlated = BFArray(np.zeros(
                        #    shape=(self.nchan, 2099200, 4),
                        #    dtype=np.complex64),
                        #    space='cuda'
                        #)
                        if self.test:
                    ...
                    #self.log.info("Correlation time for one channel (I think): %f" % perf)
                    curr_time = time.time()
                    ...
                    if this_gulp_time == last:
                        ...
                        #at end of time integration, start calculation for gridding
                        for chan_id in range(self.nchan):
                            #self.log.info("making new chan data for channel %d" % chan_id)
                            chan_data = ospan.data_view("ci32").reshape(1, self.nchan, 2099200, self.npol**2)[:, chan_id, :(self.nant*(self.nant-1)//2), :self.npol].reshape(1, 1, (self.nant*(self.nant-1)//2), self.npol)
